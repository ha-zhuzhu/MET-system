...
class A7670C:
    ...
    def __init__(self, device: str, baudrate=115200):
        ...
        self.notify_voice_path=asyncio.Queue()      # 语音文件路径队列
    ...
    async def parse_data(self, data: str):
        ...
        if "+CGATT: 1" in data:
            self.attached = True
            logging.info("A7670C attached to network")
            if not self.notify_running:
                # asyncio.create_task(self.notify_inst.put("start"))
                self.notify_running = True
        elif "+CGATT: 0" in data:
            self.attached = False
            logging.info("A7670C detached from network")
            if self.notify_running:
                self.notify_running = False
                # asyncio.create_task(self.notify_inst.put("stop"))
        elif "VOICE CALL: BEGIN" in data:
            # 电话呼入
            # 异步播放音频
            logging.info("Hang on, start playing voice")
            voice_path=await self.notify_voice_path.get()
            winsound.PlaySound(voice_path, winsound.SND_FILENAME | winsound.SND_ASYNC | winsound.SND_LOOP)
        elif "VOICE CALL: END" in data:
            # 电话挂断
            logging.info("Hang off, stop playing voice")
    ...
    async def notify_manager(self):
        ...
        while True:
            ...
            if inst == "call" and self.notify_running:
                ...
                # 异步执行cmd
                voice_path="E:\\MET\\MET-system\\ws-server\\tts\\voice\\{}.wav".format(time.time())
                cmd="echo \'{}\' | \
  E:\\MET\\MET-system\\ws-server\\tts\\piper\\piper.exe -m E:\\MET\\MET-system\\ws-server\\tts\\piper\\zh_CN-huayan-medium.onnx -c E:\\MET\\MET-system\\ws-server\\tts\\piper\\zh_CN-huayan-medium.onnx.json -f {}".format(content,voice_path)
                stdout, stderr = await execute_shell_command(cmd)
                logging.info(f"tts stdout: {stdout}")
                logging.info(f"tts stderr: {stderr}")
                asyncio.create_task(self.notify_voice_path.put(voice_path))
                # 拨打电话
                ...
                # 发送短信
                ucs2_content=await self.msg_ucs2_encode(content)
                pdu=await self.gen_pdu(number,center_phone,ucs2_content)
    ...
    async def call(self, number: str, content: str = ""):
        ...
        asyncio.create_task(self.notify_call_content.put(content))
...
# async def main():
#     a7670c=A7670C(A7670C_DEVICE)
#     await a7670c.start()
#     # await a7670c.call('13438233066','你好')    # 不需要86
#     # await a7670c.message('8613438233066','你好')  # 一定要86

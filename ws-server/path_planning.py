...
# 定义计算权重的函数
async def calculate_weight(properties):
    ...
    # 根据边类型调整权重
    """
    假设走廊速度 5m/s，所以type_factor=0.2
    楼梯2.5m/s，所以=0.4
    
    不同地方收到拥挤时，造成的效果不同
    通过type_factor = 0.1 * (1 + congestion * 2) 来实现
    （其中2为拥挤对该地形造成影响的系数）
    获取两节点间的实际物理距离，距离直接相乘
    节点进行累加
    
    或者距离与拥挤拥有另外的影响系数
    """
    if edge_type == "elevator":
        type_factor = 0.1 * (1 + congestion * 2)
    elif edge_type == "lift":
        type_factor = 0.1 * (1 + congestion * 3)
    elif edge_type == "stair":
        type_factor = 0.4 * (1 + congestion)
    elif edge_type == "corridor":
        type_factor = 0.2 * (1 + congestion)
    else:
    ...
    # 综合计算权重
    weight = distance * type_factor * status_factor * base_weight
    # weight = distance * congestion * type_factor * status_factor * base_weight
    return weight


async def add_nodes(G, filename):
    # 读取 GeoJSON 节点文件
...
async def add_edges(G, filename):
    # 读取边文件
    with open(filename, "r") as f:
    ...
    for edge in edges_data["features"]:
        ...
        # 计算权重
        weight = await calculate_weight(properties)
...
async def connect_to_nearest(G, new_point, user_id, point_pos_attr='pos'):
...
async def Dijkstra(G, start_node, end_node, my_weight="weight"):
    # 计算最短路径
...
async def generate_geojson_path3(G, shortest_path):
    # 直接生成path并返回，不写入文件
    # 获取路径坐标
    path_data = [
    ...
    start_node = path_data[0][0]
    end_node = path_data[-1][0]
...
def generate_geojson_path2(
    G, shortest_path, shortest_time_weight, shortest_path_weight
):
    # 获取路径坐标
    path_data = [
        (G.nodes[node]["pos"], G.nodes[node]["building_name"], G.nodes[node]["floor"])
